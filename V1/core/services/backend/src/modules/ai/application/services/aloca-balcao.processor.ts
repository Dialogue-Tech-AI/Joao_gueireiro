...
/**
 * Processador específico para alocabalcao.
 * Quando acionada, move o atendimento para a subdivisão "Não atribuídos" → "Encaminhamentos Balcão".
 * Adiciona as informações do resumo no interventionData (aparece no card da direita).
 */
export function createAlocaBalcaoProcessor(): FunctionCallProcessorHandler {
  ...
  return async (payload): Promise<{ output: string | null; data?: Record<string, unknown>; processed: boolean }> => {
    ...
    try {
      ...
      if (attendance.interventionType === INTERVENTION_TYPE) {
        logger.info(`${FC_NAME}: atendimento já está em Encaminhamentos Balcão, atualizando apenas interventionData`, {
          attendance_id,
          client_phone,
        });
      }
      ...
      logger.info(`${FC_NAME}: atendimento movido para Encaminhamentos Balcão`, {
        attendance_id,
        client_phone,
      ...
      logger.error(`${FC_NAME}: erro ao mover atendimento para Encaminhamentos Balcão`, {
        error: err?.message,
        attendance_id,
        client_phone,
      });
