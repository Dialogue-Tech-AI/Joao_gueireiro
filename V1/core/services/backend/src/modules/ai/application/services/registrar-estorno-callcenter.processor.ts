...
const FC_NAME = 'registrarestornocallcenter';
const INTERVENTION_TYPE = 'estorno';
...
/**
 * Processador para registrarestornocallcenter.
 * Roteia o cliente da triagem para a subdivisão "Estorno" em Intervenção humana.
 * Armazena os dados coletados em intervention_data e emite Socket.IO + Redis para tempo real.
 */
export function createRegistrarEstornoCallCenterProcessor(): FunctionCallProcessorHandler {
  ...
  return async (payload): Promise<{ output: string | null; data?: Record<string, unknown>; processed: boolean }> => {
    ...
    try {
      ...
      logger.info(`${FC_NAME}: atendimento roteado para Estorno (Intervenção humana)`, {
        attendance_id,
        client_phone,
      });
